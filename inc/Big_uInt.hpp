...
public:
  ...
  Big_uInt operator+(const Big_uInt& rhs) { Big_uInt res(*this); res += rhs; return res; }
  Big_uInt operator-(const Big_uInt& rhs);
  Big_uInt operator*(const Big_uInt& rhs) { Big_uInt res(*this); res *= rhs; return res; }
  
  Big_uInt operator/(const Big_uInt& rhs);
  Big_uInt operator/(const u_ll rhs) { Big_uInt res(*this); res /= rhs; return res; }
  ...
  Big_uInt operator%(const Big_uInt& rhs);
  u_ll operator%(const u_ll rhs) { Big_uInt res(*this); return _m_div__u_ll(rhs); }
  ...
  const Big_uInt& operator=(const Big_uInt& rhs) { Big_uInt copy = rhs; this->swap(copy); return *this; }
  const Big_uInt& operator=(Big_uInt&& rhs) { this->swap(rhs); return *this; }
  ...
  const Big_uInt& operator/=(const u_ll rhs) { (void) _m_div__u_ll(rhs); return *this; }
  const Big_uInt& operator/=(const Big_uInt& rhs);
  const Big_uInt& operator%=(const Big_uInt& rhs);
  ...
  {
    return (_m_size == 2u && _m_buffer[0] == u);
  }
...
private:
  ...
  void _m_mul(const Big_uInt& rhs)
  {
    ...
    // std::cout << "before __asm__mul:\n_m_mul: " << _m_buffer << " " << rhs._m_buffer << " " << _m_size - 1u << " " << rhs._m_size - 1u << " " << tmp << " " << product._m_buffer << "\n";
